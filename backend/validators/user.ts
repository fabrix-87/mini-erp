import { z } from 'zod';
import { validate, validateBody, validateParams } from "../middleware/validation";
...
export const UpdateUserDetailsSchema = z.object({
  params: UserIdParamSchema,
  body: UserDetailsBaseSchema.partial(),
});
 ...
 */
export const validateCreateUser = validate(CreateUserSchema, 'User creation');

/**
 * Middleware per la registrazione pubblica
 */
export const validateRegisterUser = validate(RegisterUserSchema, 'User registration');

/**
 * Middleware per il login
 */
export const validateLogin = validate(LoginSchema, 'User login');

/**
 * Middleware per il refresh token
 */
export const validateRefreshToken = validate(RefreshTokenSchema, 'Token refresh');

/**
 * Middleware per la richiesta di reset password
 */
export const validateForgotPassword = validate(ForgotPasswordSchema, 'Forgot password');

/**
 * Middleware per il reset password
 */
export const validateResetPassword = validate(ResetPasswordSchema, 'Reset password');

/**
 * Middleware per l'aggiornamento del profilo
 */
export const validateUpdateUserProfile = validate(UpdateUserProfileSchema, 'User profile update');

/**
 * Middleware per l'aggiornamento dei dettagli
 */
export const validateUpdateUserDetails = validate(UpdateUserDetailsSchema, 'User details update');

/**
 * Middleware per il cambio password
 */
export const validateChangePassword = validate(ChangePasswordSchema, 'Password change');

/**
 * Middleware per l'aggiornamento dei ruoli
 */
export const validateUpdateUserRoles = validate(UpdateUserRolesSchema, 'User roles update');

/**
 * Middleware per attivare/disattivare utente
 */
export const validateToggleUserStatus = validate(ToggleUserStatusSchema, 'Toggle user status');

/**
 * Middleware per la validazione dell'ID utente (params)
 */
export const validateUserId = validateParams(UserIdParamSchema, 'User ID validation');

/**
 * Middleware per query di ricerca utenti
 */
export const validateUserQuery = validate(UserQuerySchema, 'User query');
...
export type UpdateUserDetailsInput = z.infer<typeof UpdateUserDetailsSchema>['body'];
