...
export const ActivityIdSchema = z.object({
  id: z
    ...
    .pipe(z.number().int().positive("Activity ID non valido")),
});
...
export const ActivityParticipantIdSchema = z.object({
  id: z
    .string()
    .transform((val) => parseInt(val, 10))
    .pipe(z.number().int().positive("ID partecipante non valido")),
});
...
export const ActivityTemplateIdSchema = z.object({
  id: z
    .string()
    .transform((val) => parseInt(val, 10))
    .pipe(z.number().int().positive("ID template non valido")),
});
