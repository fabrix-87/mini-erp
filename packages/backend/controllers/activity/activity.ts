 ...
 */
export const createActivity = asyncHandler<AuthenticatedValidatedRequest>(
  async (req, res) => {
...
export const updateActivityStatus = asyncHandler(
  async (req: AuthenticatedValidatedRequest, res: Response): Promise<void> => {
    ...
    const shouldAutoEnd =
      status === "COMPLETED" && !activity.actualEnd && parsedEnd === undefined;
...
export const completeActivity = asyncHandler(
  async (req: AuthenticatedValidatedRequest, res: Response): Promise<void> => {
    ...
    const { outcome, result, requiresFollowUp, followUpDate, internalNotes } =
      req.validatedBody as CompleteActivityInput;
    ...
    let updateData = clean<Prisma.ActivityUpdateInput>({
      status: "COMPLETED",
      outcome,
      result,
      requiresFollowUp,
      internalNotes: ifDefined(internalNotes, (v) => v),
      followUpDate: ifDefined(followUpDate as any, (v) => toDate(v)),
      actualEnd: new Date(),
    });

    if (!activity.actualStart) {
      updateData = clean({ ...updateData, actualStart: new Date() });
    }
