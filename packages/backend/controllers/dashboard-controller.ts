...
import { getValidatedBody, getValidatedQuery } from "@/helpers/validated-context";
 ...
 */
export const getUnifiedDashboard = async (c: Context<AppBindings>) => {
...
export const updateDashboardLayout = async (c: Context<AppBindings>) => {
  ...
  const allowedWidgets = getAllowedWidgets(userRoles);
  const invalidWidgets = widgets.filter((w) => !allowedWidgets.includes(w.widgetType));
  ...
  return sendSuccess(c, { message: "Layout salvato con successo" });
};
 ...
 * @access Private (all authenticated users)
 */
export const resetDashboardLayout = async (c: Context<AppBindings>) => {
  const userId = c.get("user")!.userId;
  ...
  return sendSuccess(c, { message: "Layout ripristinato al default" });
};
